...
def run_scenario(name: str, cfg: SimConfig, out_dir: Path) -> dict:
    ...
    # Save CSV
    df.to_csv(out_dir / f"verification_{name}_portfolio.csv", index=False, lineterminator="\n")
...
def main(argv=None):
    ...
    summary_df.to_csv(out_dir / "verification_summary.csv", index=False, lineterminator="\n")
    ...
    (out_dir / "verification_results.json").write_text(
        json.dumps(json_results, indent=2, default=str), encoding="utf-8", newline="\n"
    )
